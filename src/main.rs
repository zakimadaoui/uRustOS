...
impl OSThread{ 
    pub fn new(stack_arr: &mut [u32], thread_runnable: fn()) -> Self{
        ...
        let mut sp: u32 = 
            stack_begin + 
            (stack_arr.len()*4) as u32;  /* times 4  because a word is 4x1byte */
...
static mut thread_curr : *mut OSThread = 0 as *mut OSThread; //TODO: use voletile to access these, remember you are in interrupts world
static mut thread_next : *mut OSThread = 0 as *mut OSThread; //TODO: use voletile to access these, remember you are in interrupts world

static mut m_thread1 : *mut OSThread = 0 as *mut OSThread; 
static mut m_thread2 : *mut OSThread = 0 as *mut OSThread; 
...
fn generate_soft_irq(){
    ...
    unsafe{//TODO: make this atomic OR operation (if there is an instruction or almost atomic)
...
unsafe fn OS_Sched(/*Add cs proof later */){
...
fn thread1_run(){
    ...
    unsafe{
        loop {
            G_COUNT=id;
            id+=2;
            OS_Sched(); //simulate an interrupt
        }
    }
}
...
fn blink() -> ! {
    let mut led;
    unsafe {led = G_LED.as_mut().unwrap();}
